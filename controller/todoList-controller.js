...
const updateTodosByUid = async (req, res, next) => {
    ...
    const updated = await todoListSchema.findOneAndUpdate({ "creator": uid, todos: { $elemMatch: { "_id": id } } }, {
        "$set": {
...
const getTodoListById = async (req, res, next) => {
    ...
    res.json({ result: result.todos.map((itr) => itr.toObject({ getters: true })) });
}
...
const getTodoListByUser = async (req, res, next) => {
    ...
    res.status(200).json({ result: getWithUser.todos[0].todos.map((itr) => itr.toObject({ getters: true })) });
}
...
const getTodoListByUid = async (req, res, next) => {
    ...
    if (!getData) return res.status(200).json({ message: "No Todos Found" });
...
const deleteAllTodos = async (req, res, next) => {
    ...
    try {
        deleteTodo = await todoListSchema.findOne({ "creator": uid }).populate("creator");
    } catch (error) {
...
const deleteOne = async (req, res, next) => {
    ...
    try {
        ...
        }, { 'safe': true }, (error, deletedObj) => {
            if (error) {
                console.log(error);
                return next(new httpError("Error occured while deleting Todo", 500));
            }
            if (deletedObj.nmodified) return res.status(201).json({ message: "Deleted Successfully" })
            else return res.status(404).json({ message: "Todo not found to delete" })
        })
        // console.log(deleted)
